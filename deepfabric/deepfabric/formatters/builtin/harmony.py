...
class HarmonyFormatter(BaseFormatter):
    ...
    def validate(self, entry: dict[str, Any]) -> bool:
        ...
        if "messages" in entry:
            ...
            # Should have at least one user message (and optionally assistant for existing conversations)
            roles = [msg.get("role") for msg in messages if isinstance(msg, dict)]
            has_user = "user" in roles
            has_assistant = "assistant" in roles

            # Valid if we have user messages (for response generation) or complete conversations
            return has_user or has_assistant
